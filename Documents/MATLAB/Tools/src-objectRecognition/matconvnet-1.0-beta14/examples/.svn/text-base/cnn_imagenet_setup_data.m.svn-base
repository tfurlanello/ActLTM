...
opts.dataDir = fullfile('data','imagenet10') ;
opts.lite = false ;
...
if numel(names) ~= 1261406 % 1281167(2012)
  warning('Found %d training images instead of 1,261,406. Dropping training set.', numel(names)) ;
  names = {} ;
  labels =[] ;
end
...
if numel(labels) ~= 150e3 % 100e3 (2012)
  warning('Found %d instead of 150,000 test images', numel(labels))
end
