...
opts.orderDir = fullfile('data','order') ;
opts.continue = false ;
opts.batchSize = 256 ;
...
for epoch=start+1:opts.numEpochs
  ...
  % train one epoch
  rng('shuffle');
  trainOrderFile = fullfile(opts.orderDir, sprintf('train-order-%d.mat', epoch));
  if ~exist(trainOrderFile, 'file')
      state.train = opts.train(randperm(numel(opts.train))) ; % shuffle
  else
      trainOrder = load(trainOrderFile);
      state.train = trainOrder.trainOrder;
      clear trainOrder;
  end
  state.epoch = epoch ;
...
start = tic ;
num = 0 ;
synLayers = net.synLayers; %% added by jiaping Feb. 18, 2016
...
for t=1:opts.batchSize:numel(subset)
  ...
  for s=1:opts.numSubBatches
    ...
    if strcmp(mode, 'train')
      %% modified by jiaping on Feb. 18, 2016
       % before training, we synchronize corresponding dropout layers 
      if ~isempty(synLayers)
        assert(size(synLayers,2) == 4);
        nsyn = size(synLayers,1);
        
        for ns=1:nsyn
            ls    =  synLayers{ns,1};
            sz    =  synLayers{ns,2}; sz(4) = numel(batch);
            rate  =  synLayers{ns,3};
            scale =  synLayers{ns,4};
            if numGpus >= 1
                mask = single(scale) * single(gpuArray.rand(sz) >= rate);
            else
                mask = single(scale) * single(rand(sz) >= rate);
            end            
            for dl=1:numel(ls)
                assert(isa(net.layers(ls(dl)).block, 'dagnn.DropOut'));
                net.layers(ls(dl)).block.mask = mask;
                net.layers(ls(dl)).block.frozen = true;
            end            
        end        
      end
...
% -------------------------------------------------------------------------
updatelists = net.updatelists;
% no need to update parameters of Loss layer
for i=1:numel(net.params)
    if ~ismember(i, updatelists)
        continue;
    end
