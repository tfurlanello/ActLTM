function Y = vl_nnlossStructured(X,c, labelgraph, dzdy,varargin)
...
% opts.labelgraph         = struct('adjacency', [], 'interaction', []);
...
switch lower(opts.loss)
  case {'classerror', 'log', 'softmaxlog', 'mhinge', 'mshinge',  ...
          'crossentropy',  'classerror-crossentropy', 'prediction-crossentropy', ...
          'maxmargin',     'classerror-maxmargin', 'prediction-maxmargin'}
    binary = false ;
...
%% ===============================================================
if opts.isstructured == true

    if isempty(labelgraph)
        error('structured prediction requires a label-graph input\n');
    end
    % weight matrix
    labelAdjacency   = labelgraph.adjacency;
    labelInteraction = labelgraph.interaction;
