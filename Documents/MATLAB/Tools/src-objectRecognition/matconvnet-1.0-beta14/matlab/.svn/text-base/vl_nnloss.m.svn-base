...
% modified by Jiaping Zhao @ Oct. 5, 2015
% structured loss functions are added

opts.instanceWeights    = [] ;
opts.classWeights       = [] ;
opts.threshold          = 0 ;
opts.loss               = 'softmaxlog' ;  % max-margin, cross-entropy to be added
% fields related to structured loss 
...
switch lower(opts.loss)
  case {'classerror', 'log', 'softmaxlog', 'mhinge', 'mshinge',  ...
          'crossentropy',  'classerror-crossentropy',  ...
          'maxmargin',     'classerror-maxmargin'}
    binary = false ;
  ...
  case {'l2'}
    ...
    % null labels denote instances that should be skipped
    instanceWeights = single(ones([labelSize(1) labelSize(2) 1 labelSize(4)]));
...
%% ===============================================================
% case 1: stuctured loss, label graph
%% ===============================================================
if gather(opts.isstructured) == true

    % weight matrix
%     labelAdjacency   = opts.labelgraph.adjacency;
    labelInteraction = opts.labelgraph;
    cpu_labelInteraction = gather(labelInteraction);

 	nCategories = size(labelInteraction,1);
    weights     = gpuArray(zeros(size(X), 'single'));
    ...
    % cross-entropy loss
    % max-margin loss is to be added
     if nargin <= 2 || isempty(dzdy)
          switch lower(opts.loss)
            ...
            case 'classerror-crossentropy'  
              ...
              for i=1:nCategories
                  i_weight = reshape(cpu_labelInteraction(i,:), [1 1 nCategories 1]);
...
%% ===============================================================
% case 2: labels are exclusive
%% ===============================================================

if gather(opts.isstructured) == false
    ...
    if nargin <= 2 || isempty(dzdy)
      switch lower(opts.loss)
        ...
        case 'hinge'
          t = max(0, 1 - c.*X) ;
        %% modified by jiaping, Feb. 11, 2016          
        case 'l2'
          d = bsxfun(@minus, X, c) ;
          t = sqrt(sum(d.*d,3)) ;              
      end
