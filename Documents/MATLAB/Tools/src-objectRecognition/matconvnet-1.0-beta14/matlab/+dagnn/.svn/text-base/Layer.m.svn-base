...
%       [net.vars(out).value] = deal(outputs{:}) ;
      ...
      %% modified by jiaping zhao 10/12/2015
      % accumulate variable values, using plus operator
      for i = 1:numel(out)
        v = out(i) ;
        if net.numPendingVarRefsIn(v) == 0
          net.vars(v).value = outputs{i} ;
        else
          net.vars(v).value = net.vars(v).value + outputs{i} ;
        end
        net.numPendingVarRefsIn(v) = net.numPendingVarRefsIn(v) + 1 ;
      end
    ...
    %  non-trivially, or to optimise certain blocks.
      ...
      if net.conserveMemory
        ...
        % unless precious
        for i = out
          net.numPendingVarRefsIn(i) = net.numPendingVarRefsIn(i) - 1 ;
          
          if net.vars(i).precious, continue ; end          
          if net.numPendingVarRefsIn(i) == 0
              net.vars(i).der = [] ;
              net.vars(i).value = [] ;
          end
        end
      ...
      if isa(obj, 'dagnn.Loss') && strcmp(obj.loss, 'l2') 

        v = in(1) ;
        if net.numPendingVarRefs(v) == 0
          net.vars(v).der = derInputs{1} ;
        else
          net.vars(v).der = net.vars(v).der + derInputs{1} ;
        end
        net.numPendingVarRefs(v) = net.numPendingVarRefs(v) + 1 ;
	...
	% for loss layer, no need to update parameters
    %% modified by jiaping (Feb. 16)
      if isa(obj, 'dagnn.Loss') %&& ~strcmp(obj.loss, 'l2') 
          return;
      end
