...
for l = 1:numel(net.layers)
  ...
  switch net.layers{l}.type
    ...
    case {'softmaxloss'}
      ...
      % labelgraph parameters
      params(1).name = sprintf('%s_isstructured', name);
      params(1).value = false;
      params(2).name = sprintf('%s_labelgraph', name);
      params(2).value = [];
	case {'crossentropy'}
      block = Loss('loss', 'crossentropy');
      inputs{2} = getNewVarName(obj, 'label');
      % labelgraph parameters
      params(1).name = sprintf('%s_isstructured', name);
      params(1).value = true;
  ...
  if ~isempty(params)
    ...
    if nparams == 2 && ~isempty(params(2).value)
    ...
    if nparams == 2 && ~isempty(params(2).learningRate)
    ...
    if nparams == 2 &&  ~isempty(params(2).weightDecay)
...
if opts.canonicalNames
  for l = 1:numel(obj.layers)
    ...
    if isa(obj.layers(l).block, 'dagnn.Loss')
      obj.renameVar(obj.layers(l).outputs{1}, getNewVarName(obj, 'objective')) ;
