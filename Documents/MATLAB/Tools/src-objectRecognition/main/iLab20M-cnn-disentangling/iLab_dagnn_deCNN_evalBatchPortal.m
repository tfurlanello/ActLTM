function varargout = iLab_dagnn_deCNN_evalBatchPortal(net, imdb, subset, networkInputs, ...
...
%XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
   ...
   net.mode = 'test'; %% this is really important!!!, use 'no dropout'    
    useGpu = 1;

    opts_getbatch   = vl_argparse(dagnn_opts_getbatch, net.meta.normalization);
    getBatch        = iLab_de_getBatchDagNNWrapper(opts_getbatch, useGpu, net.inputsNames, networkInputs);
%             subset          =  find(imdb.images.set==3);
