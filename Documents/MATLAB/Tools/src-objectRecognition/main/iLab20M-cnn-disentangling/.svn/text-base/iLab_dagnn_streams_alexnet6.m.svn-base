function varargout = iLab_dagnn_streams_alexnet6(nstreams, bshare, args)  
...
% flexibility to use different manually-crafted loss functions 
    ...
    synLayers    = cell(nLayers,1);
    cntDropout   = 0; % count the number of dropout layers (in each stream)
% >>>>>>>>>>>>>>>>>> start of the for loop to build the parallel streams >>>
% >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>    
    for s=1:nstreams
    
        cntDropout = 0;  % reset the # of dropout layers to be 0
