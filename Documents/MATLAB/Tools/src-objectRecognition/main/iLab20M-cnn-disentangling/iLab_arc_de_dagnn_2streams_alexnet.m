function net = iLab_arc_de_dagnn_2streams_alexnet( nclasses, ntransformations, ...
                            param_init_net, bshare_s, bshare_lf, ...
                            nfactors_lf, fractions_lf, args, labelgraph)    
% functionality:
...
%% additional notes:
% Although this architecture has two streams, it is equivalent to the
% single stream alexnet. The intuition behind building this architecture is
% to make fair comparisons with those architectures with two streams. Their
% parameters could be initialized to be the same.

%% additional notes:
% we split fc7 into two parts, and both of them are connected to the top
% layer label layer, while in the traditional alexnet, it is a linear chain
% architecture, with the fc7 layer directly connected with the top label
% layer
...
% XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX   
   %% identity prediction1
    ...
    %% identity prediction2 
	nameLayer = 'predictionIdentity2';    
    inputs = representationsTransform{1};
    nameLayer = iLab_dagnn_getNewLayerName(net, nameLayer);
    outputs   = predictionsIdentity;
    ...
    net = iLab_dagnn_addlayer_conv(net, nameLayer, inputs, outputs, ...
                {'size', [1 1 nTransformation nclasses], 'stride', 1, 'pad', 0, ...
                'filter',           [], ...
                'bias',             [], ...
                'fname',            fname, ...
                'bname',            bname, ...
