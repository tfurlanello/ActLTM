function bb = iLab_calBB2(opts, scale)
...
%     segout = fullfile(subsaveDir, [imNames(1:end-4) '-out' imgExt]);
    ...
    %% import settings: this setting works in +90% cases
%     Ks        = [600 1000 1400 1800]* (scale^2);
%     minsize   = round(500 * (scale^2));
    
    %% this setting may get tighter bounding boxes
    ...
    parfor s=1:nSettings
        ...
        if bgPercent < 0.5
            ...
            bb =  iLab_conncomp2objBB(fgmask, objCenter_scaled);
        end
        BBs{s} = bb;
