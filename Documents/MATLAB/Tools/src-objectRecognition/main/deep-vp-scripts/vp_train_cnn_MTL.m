...
%   make sure: parameters are consistent
    ...
    param.modelType         = 'alexnet-dagnn-MTL' ;
    param.networkType       = 'dagnn';
    whoseLabels             = containers.Map({'object', 'environment'}, 1:2);
...
%     nclasses_env = 15;
    nTasks = 6;
    nTaskLabels = zeros(nTasks,1);
    arc.fc.size     = 512;
% -------------------------------------------------------------------------
%                                                              dataset info
% -------------------------------------------------------------------------
    par_imdb.dataDir = '/home2/u/kai/deep_vp/google_dataset6';
    par_imdb.lite = false ;
...
% -------------------------------------------------------------------------    
    ...
    train.sync             = false ;
    train.cudnn            = true ;
    train.expDir           = fullfile(saveDir, sprintf('vp-%s', sfx)) ;
...
if exist(par_imdb.imdbFile, 'file')
  ...
  imdb = vp_cnn_setupdata_MTL('dataDir', par_imdb.dataDir, 'lite', par_imdb.lite) ;
  mkdir(param.expDir) ;
  save(par_imdb.imdbFile, '-struct', 'imdb') ;
end

btrain = imdb.images.set == 1;
trainLabels = imdb.images.label(:, btrain);
for t=1:nTasks
    nTaskLabels(t) = numel(unique(trainLabels(t,:)));
end
...
%                                                 struct('isstructured', false, 'labelgraph', []));
    ...
    case 'alexnet-dagnn-MTL'
        net = ...
            iLab_arc_dagnn_MTL_nLabelLayers(nTasks, nTaskLabels, {}, arc, 'alexnet');

        nOutputs    =   numel(net.outputsNames);
        derOutputs  =   cell(1, 2*nOutputs);
        for o=1:2:2*nOutputs
            derOutputs{2*floor(o/2)+1} = net.outputsNames{floor(o/2)+1};
            derOutputs{2*floor(o/2)+2} = 1;
        end
        train.derOutputs   = derOutputs;
        infoPortVars = net.inputsNames;        
        whoseLabels        = containers.Map(infoPortVars(2:end), 1:(numel(infoPortVars)-1));
...
switch lower(param.networkType)
 case 'simplenn'
  ...
  case 'dagnn'
    fn = vp_getBatchDagNNWrapper_MTL(getBatch_opts, useGpu, infoPortVars, whoseLabels) ;
