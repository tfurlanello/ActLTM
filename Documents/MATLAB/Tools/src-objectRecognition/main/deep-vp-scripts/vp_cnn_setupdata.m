function imdb = vp_cnn_setupdata(varargin)
...
trainLabelsObj = trainLabels(1:1:(nTrains));
trainLabelsEnv = trainLabelsObj;
fclose(flabel);
...
for i=1:numel(names) 
    names{i} = ['/lab/igpu3' names{i}(7:end)]; 
end
...
testLabelsObj = testLabels(1:1:(nTest));
testLabelsEnv = testLabelsObj;
fclose(flabel);
