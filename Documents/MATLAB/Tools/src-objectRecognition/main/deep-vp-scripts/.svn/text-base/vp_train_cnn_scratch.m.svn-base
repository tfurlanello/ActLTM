...
%   make sure: parameters are consistent
    ...
    fcsizes = [1024 256 128 64 32 16];
    nsizes = numel(fcsizes);
    
for fc=1:1 %nsizes
    
    nclasses_obj = 225;
    arc.fc.size = fcsizes(fc);
% -------------------------------------------------------------------------
%                                                              dataset info
% -------------------------------------------------------------------------
    par_imdb.dataDir = '/lab/igpu3/u/kai/deep_vp/google_dataset8';
    par_imdb.lite = false ;
    saveDir = par_imdb.dataDir;
    saveDir = '/lab/igpu3/u/kai/deep_vp/results/google_dataset-grey-1M';    
    ...
    expDir = fullfile(saveDir, sprintf('vp-%s-fc%d-color', sfx, arc.fc.size)) ;
