...
MTL_xy = '/home2/u/kai/deep_vp/results/google_dataset-grey-1M/vp-alexnet-dagnn-multiLevelInjection-fc2/visualization';
...
tSNEsaveName =  'tSNE-fc2-2D.mat';

% w2cnn_MI
for a = 1:1
    a
    if exist(fullfile(MTL_xy,  tSNEsaveName), 'file')
        continue;
    end
    
    evalFile = fullfile(MTL_xy, evalFileName);
    ...
    sparseIdx = rorder(startIdx:gap:nImgs); 
...
%     fc22D = fc22D(landmarks,:);
    ...
    save(fullfile(MTL_xy,  tSNEsaveName), '-struct', 'tSNEfc2' , '-v7.3');
    clear tSNEfc2;
