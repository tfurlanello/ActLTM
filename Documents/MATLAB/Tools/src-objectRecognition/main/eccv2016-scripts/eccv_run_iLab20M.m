...
fundamentals = [7 11 18 56];
nF           = numel(fundamentals);
rootSaveDir  = '/lab/igpu3/u/jiaping/iLab20M-objRec/CNN-results/ECCV2016/exp2';
dataDir = '/lab/igpu3/u/jiaping/iLab20M-objRec/dataset/iLab20M-datasets-pilot-experiments/category-camera'; 
rootTrainOrderDir = '/lab/igpu3/u/jiaping/iLab20M-objRec/CNN-results/ECCV2016/training-orders';


nfactors_lf     = 2;
fractions_lf    = [1/2 1/2];
...
for f=1:nF
    ...
    %   make sure: parameters are consistent
        ...
        nclasses_obj = 10;
        nclasses_transformation = fundamentals(f);
        bshare = true; % two streams of alexnet share the same convolutional parameters
        bshare_lf = true;

        trainOrderDir = fullfile(rootTrainOrderDir, ['f' num2str(fundamentals(f))]);
        if ~exist(trainOrderDir, 'dir')
            mkdir(trainOrderDir);
        end
        saveDir = fullfile(rootSaveDir, ['f' num2str(fundamentals(f))]);
    ...
    % -------------------------------------------------------------------------    
        ...
        if ~fBatchNormalization
          train.learningRate = logspace(-2, -4, 6) ; %% use only 6 epochs
        else
    ...
    % -------------------------------------------------------------------------
        if exist(par_imdb.imdbFile, 'file')
          ...
          imdb = iLab_de_cnn_setupdata('dataDir', par_imdb.dataDir, 'lite', par_imdb.lite, 'nf', fundamentals(f)) ;
    ...
    % -------------------------------------------------------------------------
        if ~exist(fullfile(rootSaveDir, netInitParamFileName), 'file')
            ...
            save(fullfile(rootSaveDir, netInitParamFileName), 'net_init_param') ;
        end
        if exist(fullfile(rootSaveDir, netInitParamFileName), 'file')
            load(fullfile(rootSaveDir, netInitParamFileName));
        ...
        switch param.modelType
            ...
            case 'iLab_arc_de_dagnn_2streams_woL2'
                ...
                net = iLab_arc_de_dagnn_2streams_woL2(nclasses_obj, nclasses_transformation,...
                                                net_init_param, bshare, bshare_lf, ...
                                                nfactors_lf, fractions_lf, arc, struct('isstructured', false, 'labelgraph', [])); 
