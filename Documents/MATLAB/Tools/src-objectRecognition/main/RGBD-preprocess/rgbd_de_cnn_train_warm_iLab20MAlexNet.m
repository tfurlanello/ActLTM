%% notes:
% fine-tune object recognition, using the pre-trained model
% the pre-trained model is AlexNet on iLab20M dataset
...
arc_currents     = [1 3 2];          
bmemory          = true; 
balancingFactors = {[1 1], 1, [0.05 1 1]};
...
for rep=1:nReps
    ...
    % -------------------------------------------------------------------------
        ...
        par_imdb.lite = false ;
        saveDir = '/lab/igpu3/u/jiaping/washington-RGBD/results/ECCV/warmstart-iLab20M-AlexNet/exp1';
        if ~exist(saveDir, 'dir')
            mkdir(saveDir);
        end
        trainorderDir = '/lab/igpu3/u/jiaping/washington-RGBD/results/ECCV/training-orders';
        trainorderDir = fullfile(trainorderDir, ['rep-' num2str(repetitions(rep))]);
    ...
    % -------------------------------------------------------------------------    
        ...
        if ~fBatchNormalization
          train.learningRate = logspace(-2, -4, 10) ;
        else
        ...
        switch param.modelType
            ...
            case 'iLab_arc_de_dagnn_2streams_woL2'
                ...
                net = iLab_arc_de_dagnn_2streams_woL2(nclasses_obj, nclasses_transformation,...
                                                net_init_param, bshare, bshare_lf, arc, struct('isstructured', false, 'labelgraph', [])); 
